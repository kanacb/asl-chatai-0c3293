...
const SingleSessionsPage = (props) => {
  ...
  useEffect(() => {
    ...
    client
      .service("sessions")
      .get(urlParams.singleSessionsId, {
        query: {
          $populate: [
            {
              path: "createdBy",
      ...
      .catch((error) => {
        ...
        props.alert({
          title: "Sessions",
          type: "error",
          message: error.message || "Failed get sessions",
        });
      });
    ...
    <div className="col-12 flex flex-column align-items-center">
      ...
      <div className="grid col-10">
        <div className="card w-full">
          ...
          <div className="ml-3">
            <p className="m-0 ml-3">{_entity?.loginSession}</p>
          </div>
          <label className="text-sm text-primary">UserId</label>
          <div className="ml-3">
            <p className="m-0 ml-3">{_entity?.userid?.name}</p>
          </div>
