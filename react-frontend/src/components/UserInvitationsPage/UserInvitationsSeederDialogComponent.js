...
import seederData from "./userInvitations.seeder.json";
...
const UserInvitationsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("userInvitations").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(UserInvitationsSeederDialogComponent);
