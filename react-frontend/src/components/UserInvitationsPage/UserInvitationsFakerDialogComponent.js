...
import userInvitationsFakerFactory from "./userInvitationsFakerFactory";
...
const UserInvitationsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = userInvitationsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("userInvitations").create(elem));
...
export default connect(mapState, mapDispatch)(UserInvitationsFakerDialogComponent);
